...
#include "network/download.h" // NOLINT(misc-include-cleaner)
#include "statistics.h"       // NOLINT(misc-include-cleaner)
...
inline bool IsSmaller(const catalog::DirectoryEntry& a,
  ...
  const bool a_is_first = (a.inode() == catalog::DirectoryEntryBase::kInvalidInode);
  const bool a_is_last = (a.inode() == kLastInode);
  const bool b_is_first = (b.inode() == catalog::DirectoryEntryBase::kInvalidInode);
