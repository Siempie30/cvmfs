  ...
  upload::Spooler   *spooler)
{
  ...
  {
    const UniquePtr<CatalogDatabase> new_clg_db(CatalogDatabase::Create(file_path));
...
void WritableCatalogManager::Clone(const std::string &destination,
  ...
  const bool destination_already_present =
  ...
  const NameString &dest_name)
{
  ...
  for (unsigned i = 0; i < ls.size(); ++i) {
    ...
    if (ls[i].IsChunkedFile()) {
      ...
      const std::string relative_sub_path = MakeRelativePath(sub_path.ToString());
  ...
  const std::string     &parent_directory)
{
  ...
  const unsigned mbytes = entry.size() / static_cast<uint64_t>(1024 * 1024);
  ...
  const FileChunkList &file_chunks)
{
  ...
  const unsigned mbytes = entries[0].size() / static_cast<uint64_t>(1024 * 1024);
  if ((file_mbyte_limit_ > 0) && (mbytes > file_mbyte_limit_)) {
    LogCvmfs(kLogCatalog, kLogStderr,
...
void WritableCatalogManager::SwapNestedCatalog(const string &mountpoint,
  ...
  const UniquePtr<Catalog> new_catalog(LoadFreeCatalog(nested_root_ps, new_hash));
                                                ...
                                                const uint64_t new_size)
{
  ...
  const DeltaCounters delta = Counters::Diff(counters, new_catalog->GetCounters());
...
void WritableCatalogManager::FinalizeCatalog(WritableCatalog *catalog,
  ...
  const uint64_t catalog_limit = static_cast<uint64_t>(1000) *
    static_cast<uint64_t>(catalog->IsRoot()
...
void WritableCatalogManager::CatalogUploadCallback(
  ...
  {
    const MutexLockGuard guard(catalog_processing_lock_);
    const std::map<std::string, WritableCatalog*>::iterator c =
