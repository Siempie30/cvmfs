  ...
  , history_(NULL)
{
  ...
  if (exists) {
    int rvb;
    const std::string keys = JoinStrings(FindFilesBySuffix(
  ...
  const std::string &url, const std::string &fqrn, const std::string &tmp_dir)
{
  ...
  if (!manifest_->history().IsNull()) {
    const std::string tags_url = url + "/data/" + manifest_->history().MakePath();
...
void Publisher::CreateDirectoryAsOwner(const std::string &path, int mode)
{
  ...
  const int rvi = chown(path.c_str(), settings_.owner_uid(), settings_.owner_gid());
...
void Publisher::ExitShell() {
  ...
  const pid_t pid_child = static_cast<pid_t>(String2Uint64(session_pid));
