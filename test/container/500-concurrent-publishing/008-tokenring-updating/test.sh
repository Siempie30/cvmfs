...
echo "\n---Removing gateway 3"
execute_in_container cvmfs-gw3 "systemctl stop cvmfs-gateway" || exit 12
...
echo "\n---Verifying updated token ring tables"
EXPECTED="http://cvmfs-gw1:4929/api/v1|test.repo.org|0
http://cvmfs-gw3:4929/api/v1|test.repo.org|3
http://cvmfs-gw2:4929/api/v1|test.repo.org|0"
OUTPUT=$(execute_in_container cvmfs-gw1 "sqlite3 $DB_FILE \"SELECT * FROM TokenRing;\"")
echo "$OUTPUT"
if [ "$OUTPUT" != "$EXPECTED" ]; then exit 14; fi
OUTPUT=$(execute_in_container cvmfs-gw2 "sqlite3 $DB_FILE \"SELECT * FROM TokenRing;\"")
echo "$OUTPUT"
if [ "$OUTPUT" != "$EXPECTED" ]; then exit 15; fi
...
echo "\n---Stopping gateway 2"
execute_in_container cvmfs-gw2 "systemctl stop cvmfs-gateway" || exit 18
