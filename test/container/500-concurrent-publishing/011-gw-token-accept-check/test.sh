...
response=$(docker exec -it cvmfs-gw1 curl -s -X POST --data '{"repo":"fake.repo.org"}' http://cvmfs-gw1:4929/api/v1/hagroup/creation | jq '.acknowledgement')
if [ "$response" != "\"error\"" ]; then exit 3; fi

sleep 2

echo "\n---Handing out token to gateway 1 (again)"
response=$(docker exec -it cvmfs-gw1 curl -s -X POST --data '{"repo":"test.repo.org"}' http://cvmfs-gw1:4929/api/v1/hagroup | jq '.acknowledgement')
